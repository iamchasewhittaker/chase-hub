...
## 2026-05-15 — Auth: email+password migration + env-var hardening
...
### Fixed
- **Preview deploys no longer silently 500.** Replaced `process.env.NEXT_PUBLIC_SUPABASE_*!` non-null assertions in `proxy.ts`, `lib/supabase-server.ts`, and `lib/supabase-browser.ts` with explicit `requireSupabaseEnv()` checks that throw a descriptive error naming which var is missing. The Supabase env vars were configured for Production only, so the first preview deploy of this branch crashed inside the middleware with a generic Internal Server Error. Added env vars to Preview (and Development) environments in Vercel; the new error message makes the next gap loud instead of silent.

### Verified
- Preview deploy login flow end-to-end with real credentials before merging PR #1. Production deploy (`daddf92`) returns HTTP 200 on `/login` with the new copy "Admin only. Enter the email and password on file."
